...
if st.session_state.user_email is None:
    st.write("Por favor, insira seu e-mail para continuar.")
    email_input = st.text_input(
        "E-mail", type="default", placeholder="seuemail@exemplo.com"
    )

    # Validate the email input and set it in session state
    ...
    ## Checkbox for default information
    ...
    genero = st.selectbox(
        "Gênero",
        ["Feminino", "Masculino"],
        index=0 if default_genero == "Feminino" else 1,
    )
    problemas = st.text_area(
        "Problemas Relatados", value=default_problemas, max_chars=300
    )
    alergias = st.text_input("Alergias", value=default_alergias, max_chars=300)
    medicacoes = st.text_input(
        "Medicações em Uso", value=default_medicacoes, max_chars=300
    )
    ...
    if st.button("Obter Diagnóstico"):
        ...
        with st.spinner("Consultando o diagnóstico..."):
            ...
            save_to_google_sheet(sheet, gpt_dict, st.session_state.user_email)
